...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[4]").setAppName("wikipedia")   // local app with 4 threads
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((_, a) => a.text.split(" ").count(x => x.equalsIgnoreCase(lang)), _ + _)
  }
