...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def rec(chL: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] =
        chL match {
          case Nil => acc
          case x :: xs => rec(xs.filter(_ != x), (x, chL.count(_ == x)) :: acc)
        }
  ...
  /*
    trees match {
      case Nil => trees
      case x :: Nil => trees
      case left :: right :: xs => (makeCodeTree(a, b) :: trees.tail.tail).sortBy(weight(_))
    }
  ... or I can rewrite like this since tree is the same for the first and the second
  */
    trees match {
      case left :: right :: xs => (makeCodeTree(left, right) :: trees.tail.tail).sortBy(weight(_))
      case _ => trees
    }
   ...
   */
  def until[T](t: (List[T]) => Boolean,  f: (List[T]) => List[T])(list: List[T]): List[T] = {
    if( t(list) ) list
    else until(t, f)(f(list))
  }
